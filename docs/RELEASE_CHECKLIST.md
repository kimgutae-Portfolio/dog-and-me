...
4. 변하지 않아야 할 특징이 최대 3개로 제한되고, 영화적 재구성 확인 없이는 접수되지 않는지 확인한다.
...
7. 관리자에서 대표 사진과 추억별 연결 사진을 검토하고 `승인` 전에는 컨셉 공개·제작 상태 전환·제작용 JSON 복사가 모두 거절되는지 확인한다.
8. 사진 보완 요청 후 고객이 사진을 수정하면 분석 상태가 다시 검토 대기로 돌아가는지 확인하고, 최종 승인 시각과 승인 운영자가 기록되는지 확인한다.
9. 입금 확인 전에는 `production`과 확인 영상 업로드가 거절되는지 확인한다.
10. 입금 확인 후 컨셉 2안을 공개하고 고객이 1안을 선택한다.
11. 확인 영상을 공개하고 고객이 수정 요청을 보낸다.
12. 미처리 수정이 남은 상태에서 고객 확정과 최종 납품이 모두 거절되는지 확인한다.
13. 수정 영상을 공개한 후 요청을 처리 완료로 바꾼다.
14. 고객이 「この映像で確定する」를 누르고, 확정 시각과 대상 영상 ID를 확인한다.
15. 최종 영상을 등록하고 납품한다.
16. 최종 영상 등록 뒤 납품만 실패시키는 테스트를 해, 등록된 영상을 재사용할 수 있는지 확인한다.
17. 고객 전용 사이트와 전용 URL이 정상이고 검색 제외 상태인지 확인한다.

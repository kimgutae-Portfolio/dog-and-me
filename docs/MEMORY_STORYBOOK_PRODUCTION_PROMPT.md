...
이 문서는 `order.json`과 `stories/*/original/` 사진을 AI에 전달할 때 사용하는 전체 의뢰문이다. 관리자 ZIP에도 `MEMORY_STORYBOOK_PRODUCTION_v2.txt`로 포함된다.
...
5. 고객 원본 사진을 최종 영상처럼 그대로 영상 생성 AI에 보내지는 않지만, 그림책 페이지를 만들 때는 강아지 정체성의 강한 이미지 레퍼런스로 사용한다.
...
- 각 물語마다 새로운 16:9 그림책 페이지 1장을 계획하고, 그 페이지를 바탕으로 영상 생성 AI용 5초 프롬프트 1개를 작성한다.
...
- 연결 페이지마다 영상 생성 AI용 5초 프롬프트를 작성하고, 페이지 자체의 작은 움직임만 지정한다.

## 동영상 제작 규칙

- 이야기 페이지: 승인된 16:9 그림책 페이지 이미지 + 영상 생성 AI + 5초.
- 연결 페이지: 승인된 16:9 배경 페이지 이미지 + 영상 생성 AI + 5초.
- raw 고객 사진은 최종 영상 생성 입력으로 사용하지 않지만, 그림책 페이지 생성 단계에서는 동일 개체를 유지하기 위한 핵심 레퍼런스로 사용한다.
...
5. `video_scene_prompts`: 5개 이야기 페이지의 영상 생성 AI용 5초 프롬프트
6. `video_transition_prompts`: 4개 연결 페이지의 영상 생성 AI용 5초 프롬프트

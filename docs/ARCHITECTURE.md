...
手動制作の内部工程は、お客様に見せる注文状態とは分けて管理します。受付点検、写真分析、映像構成案2案、選択案の制作ガイド、4〜5秒の外見テスト、シーン別生成・検収、編集QCの順に進め、前工程が承認されるまで次工程を開きません。詳細な保存データ、画面順序、判定基準は [MANUAL_PRODUCTION_WORKFLOW.md](./MANUAL_PRODUCTION_WORKFLOW.md) を正とします。

相談受付は基準写真3枚（顔・全身・横向き）と思い出3項目の入力を確認した後に確定します。お客様には3つのエピソードだけを伺い、運営者が各エピソードから複数の場面を組み立てて映像構成案2案を作成します。DBは移行中の注文との互換性のため2〜6項目・各項目の写真0〜5枚を受け付けますが、現行フォームと運用基準は3項目固定です。思い出本文と写真は同じIDで紐付き、管理者が場面ごとに確認できます。先着10組の料金枠は`materials_submitted`になった注文だけが使用するため、アップロード途中の下書き注文は枠を消費しません。

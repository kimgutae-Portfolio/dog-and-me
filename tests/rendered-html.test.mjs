...
test("server-renders the Japanese landing page", async () => {
  ...
  assert.match(html, /うちの子専用ホームページ/);
  assert.match(html, /水彩絵本ページ5枚/);
  assert.match(html, /写真アルバム/);
  assert.match(html, /ミニキャラクター/);
  assert.match(
    html,
...
test("keeps delivered albums growing from one Studio photo manager", async () => {
  ...
  assert.match(manager, /const target = managedPhotos\[index \+ direction\]/);
  assert.doesNotMatch(
    manager,
    /asset\.category === "source_image" && \([\s\S]{0,180}album-manager-visibility/,
  );

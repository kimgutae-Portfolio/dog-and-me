...
このリポジトリでは「会員登録 → 3〜5つの物語と物語ごとの場面写真を提出 → 素材確認 → 物語案2案 → 1案選択 → 絵本ページと文章の承認 → 制作・修正 → 完成した動く絵本と専用サイトの納品」までをSupabaseへ接続しています。映像制作そのものは品質を優先して手動で行い、運営画面から完成映像を納品します。

## 1次版でできること

- 日本語のブランドランディングページ
- 料金・制作フロー・FAQの確認
- 3ステップのストーリー入力（物語3〜5項目・各物語の写真1〜3枚）
...
- 初期20組 ¥16,800（税込）、その後 ¥19,800（税込）のデータベース側自動判定
- 初期20組の制作時間・動画制作ツールの使用クレジット・再生成回数を運営者だけが記録
...
Stripe Dashboardの本番環境でWebhook送信先を`https://www.wanmemory.com/api/webhooks/stripe`に設定し、`checkout.session.completed`、`checkout.session.async_payment_succeeded`、`checkout.session.async_payment_failed`、`checkout.session.expired`、`charge.refunded`を購読します。本番エンドポイントの署名シークレットを`STRIPE_WEBHOOK_SECRET`、制限付きでないサーバー用本番シークレットキーを`STRIPE_SECRET_KEY`としてVercelのProduction環境だけに登録し、`STRIPE_MODE=live`を設定します。Previewとローカルではテストキー、テスト用Webhook署名シークレット、`STRIPE_MODE=test`を使用します。どの秘密値にも`NEXT_PUBLIC_`を付けません。アプリはキー、Checkout Session、Webhookイベントのモードが一致しない場合に決済処理を拒否します。
...
- 外部の動画生成AIでの映像制作（使用量は管理画面に手動記録）
- 完成映像の管理画面アップロード
- お客様から運営者へのメール通知

AI生成APIの自動化は、初期20件の実制作時間と原価を確認した後に接続します。

...
このリポジトリでは「会員登録 → 基準写真と3つの思い出の提出 → 素材確認 → コンセプト2案 → 1案選択 → 場面イメージ承認 → 制作・修正 → 映画と専用サイトの納品」までをSupabaseへ接続しています。映像制作そのものは品質を優先して手動で行い、運営画面から完成映像を納品します。
...
4. テスト注文で入金確認 → コンセプト選択 → 場面イメージ承認 → 制作 → 確認映像 → 顧客確定 → 最終納品を確認

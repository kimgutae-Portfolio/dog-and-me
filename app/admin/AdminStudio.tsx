...
export function AdminStudio() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      setDeliveryInputKey((current) => current + 1);
  ...
  const changeFilter = (nextFilter: string) => {
    ...
    const nextOrders = nextFilter === "all" ? orders : orders.filter((item) => item.status === nextFilter);
  ...
  const selectFinalVideo = (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const deliverFinalVideo = async () => {
    const file = deliveryFile;
    if (!file || !order) return;
    if (!deliveryChecked) { setError("お客様名とファイル名を確認してください。"); return; }
    setSaving(true);
    setError("");
    const supabase = getSupabaseBrowserClient();
    ...
    if (!deliveryError) {
      const { error: orderError } = await supabase.from("orders").update({ status: "delivered", stage_updated_at: new Date().toISOString() }).eq("id", order.id);
      if (orderError) setError("映像は保存されましたが、注文状態を納品済みに変更できませんでした。");
      else {
  ...
  return (
    <main className="admin-page">
      ...
      <div className="admin-shell">
        <aside className="admin-sidebar"><p className="eyebrow">ORDERS</p><h1>制作管理</h1><select aria-label="注文の状態で絞り込む" value={filter} onChange={(event) => changeFilter(event.target.value)}><option value="all">すべての注文</option>{statusOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select><label className="admin-mobile-order-picker"><span>対応する注文</span><select value={selectedOrderId} onChange={(event) => setSelectedOrderId(event.target.value)}>{visibleOrders.map((item) => <option value={item.id} key={item.id}>{item.pet_name} · {ORDER_STATUS_LABELS[item.status]}</option>)}</select></label><div className="admin-order-list">{visibleOrders.map((item) => <button type="button" className={item.id === selectedOrderId ? "active" : ""} onClick={() => setSelectedOrderId(item.id)} key={item.id}><span>{ORDER_STATUS_LABELS[item.status]}</span><strong>{item.pet_name}</strong><small>{item.order_number} · ¥{new Intl.NumberFormat("ja-JP").format(item.quoted_price)}</small></button>)}</div></aside>
        <section className="admin-main">
          ...
          {!order ? <div className="admin-empty"><h2>注文はまだありません。</h2><p>新しい相談が入るとこちらに表示されます。</p></div> : <>
            ...
            <nav className="admin-mobile-sections" aria-label="管理項目"><a href="#admin-progress">進行</a><a href="#admin-story">内容</a><a href="#admin-concepts">2案</a><a href="#admin-delivery">納品</a><a href="#admin-message">連絡</a></nav>
            ...
            <section className="admin-card" id="admin-delivery"><div className="card-head"><div><p className="eyebrow">FINAL DELIVERY</p><h3>完成映像を納品</h3></div><span>MP4 / MOV / WebM</span></div><div className="admin-form-grid"><label><span>映画タイトル</span><input value={deliveryTitle} onChange={(event) => setDeliveryTitle(event.target.value)} /></label><label className="wide"><span>お客様へのメッセージ</span><textarea rows={3} value={deliveryMessage} onChange={(event) => setDeliveryMessage(event.target.value)} /></label></div><label className={saving ? "admin-video-upload disabled" : "admin-video-upload"}><input key={deliveryInputKey} type="file" accept="video/mp4,video/quicktime,video/webm" disabled={saving} onChange={selectFinalVideo} /><strong>{deliveryFile ? "別の完成映像を選ぶ" : "完成映像を選ぶ"}</strong><small>選択しただけでは納品されません。次の確認画面で内容を確定します。</small></label>{deliveryFile && <div className="admin-delivery-review" role="group" aria-label="納品内容の最終確認"><p className="eyebrow">DELIVERY CHECK</p><h4>まだ納品されていません</h4><dl><div><dt>お客様</dt><dd>{order.pet_name}ちゃん · {customer?.full_name || customer?.email || "登録ユーザー"}</dd></div><div><dt>ファイル</dt><dd>{deliveryFile.name}</dd></div><div><dt>サイズ</dt><dd>{(deliveryFile.size / 1024 / 1024).toFixed(1)} MB</dd></div><div><dt>タイトル</dt><dd>{deliveryTitle || `${order.pet_name}との映画`}</dd></div></dl><label className="admin-delivery-check"><input type="checkbox" checked={deliveryChecked} onChange={(event) => setDeliveryChecked(event.target.checked)} /><span>お客様名・ファイル名・タイトルを確認しました</span></label><div><button className="button button-outline" type="button" disabled={saving} onClick={clearFinalVideo}>選び直す</button><button className="button button-primary" type="button" disabled={saving || !deliveryChecked} onClick={deliverFinalVideo}>{saving ? "アップロード中…" : "確認した内容で納品する →"}</button></div></div>}</section>

            <section className="admin-card" id="admin-message"><div className="card-head"><div><p className="eyebrow">MESSAGE</p><h3>お客様へ連絡</h3></div></div><form className="admin-message-form" onSubmit={sendMessage}><textarea name="body" rows={4} placeholder="追加写真のお願い、確認事項、進行状況など" /><button className="button button-outline" type="submit">メッセージを送る</button></form></section>
          </>}
        </section>
      </div>
    </main>

...
export function SharedMemorySite() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!finalVideoUrl) {
        setError("映像を表示できません。共有URLの有効状態をご確認ください。");
        setLoading(false);
        return;
      }
      setMemory(loaded);
  ...
  return (
    <main className="private-film-page shared-memory-page">
      <header className="private-film-nav"><Link className="brand" href="/"><span className="brand-mark">WM</span><span className="brand-type">WAN MEMORY<small>FAMILY MEMORY SITE</small></span></Link><span className="shared-memory-badge">家族共有ページ</span></header>
      ...
      {images.length > 0 && <section className="private-film-section private-film-gallery"><div className="private-film-heading"><div><p>PHOTO MEMORIES</p><h2>何度でも開ける、<br />家族のアルバム。</h2></div><span>{images.length} PHOTOS</span></div><div>{images.map((image) => <figure key={image.id}><img src={image.url} alt={`${order.pet_name}の思い出写真`} draggable={false} onContextMenu={(event) => event.preventDefault()} /><figcaption>{image.caption || `${order.pet_name}との思い出`}</figcaption></figure>)}</div></section>}

...
export function MemoryMotion() {
  useEffect(() => {
    ...
    const updatePageMotion = () => {
      ...
      const rotateAmount = mobile ? 3.2 : 5.2;
      const shiftAmount = mobile ? 18 : 34;
      ...
      pages.forEach((page) => {
        ...
        const rotate = (1 - incoming) * rotateAmount - outgoing * rotateAmount * 1.15;
        const shift = (1 - incoming) * shiftAmount - outgoing * shiftAmount * 0.8;
        const scale = 1 - fold * (mobile ? 0.006 : 0.01);

        page.style.setProperty("--memory-page-rotate-y", `${rotate.toFixed(3)}deg`);
        page.style.setProperty("--memory-page-shift-x", `${shift.toFixed(2)}px`);
        page.style.setProperty("--memory-page-scale", scale.toFixed(4));
        page.style.setProperty("--memory-page-fold", (fold * (mobile ? 0.3 : 0.4)).toFixed(3));
        page.style.transformOrigin = outgoing > 0 ? "0% 50%" : "100% 50%";
      });
    };

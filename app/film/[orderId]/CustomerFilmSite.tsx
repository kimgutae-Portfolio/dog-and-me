...
export function CustomerFilmSite() {
  ...
  if (authLoading || loading || !user) return <div className="wizard-loading">大切な映像を準備しています…</div>;
  if (error || !order) return <main className="film-private-error"><p>{error || "映像が見つかりません。"}</p><Link href="/studio">制作室へ戻る</Link></main>;
  ...
  return (
    <main className="private-film-page">
      ...
      <section className="private-film-hero"><div className="private-film-glow" /><div><p>PRIVATE MEMORY FILM · {order.order_number}</p><h1>{delivery?.title || `${order.pet_name}とのメモリーフィルム`}</h1><span>{order.breed} · {order.purpose}</span></div></section>

      <section className="private-film-section"><div className="private-film-heading"><div><p>YOUR MEMORY FILM</p><h2>一緒に過ごした時間を、<br />一本のメモリーフィルムに。</h2></div><span>閲覧専用 · 有効期限付き表示</span></div><div className="private-video-frame">{videoUrl ? <video src={videoUrl} controls controlsList="nodownload noplaybackrate" disablePictureInPicture playsInline onContextMenu={(event) => event.preventDefault()} /> : <div>完成映像を準備しています</div>}</div><p className="private-video-note">ダウンロード操作は提供していません。画面録画などを技術的に完全に防ぐことはできません。</p></section>
      ...
      {images.length > 0 && <section className="private-film-section private-film-gallery"><div className="private-film-heading"><div><p>PHOTO MEMORIES</p><h2>映像になる前の、<br />大切な一枚一枚。</h2></div><span>{images.length} PHOTOS</span></div><div>{images.map((image) => <figure key={image.id}><img src={image.url} alt={`${order.pet_name}の思い出写真`} draggable={false} onContextMenu={(event) => event.preventDefault()} /><figcaption>{image.caption || `${order.pet_name}との思い出`}</figcaption></figure>)}</div></section>}

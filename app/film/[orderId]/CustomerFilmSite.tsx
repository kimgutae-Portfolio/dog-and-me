...
export function CustomerFilmSite() {
  ...
  useEffect(() => {
    if (!authLoading && !user)
      router.replace(
        `/auth?next=${encodeURIComponent(`/film/${params.orderId}`)}`,
      );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [orderResult, deliveryResult, assetsResult, conceptsResult] =
        await Promise.all([
          supabase
            .from("orders")
            .select("*")
      ...
      setConcept(
        loadedConcepts.find(
          (item) => item.slot === loadedOrder.selected_concept_slot,
        ) ?? null,
      );

      const finalAsset = loadedDelivery
        ? loadedAssets.find(
            (asset) => asset.id === loadedDelivery.final_asset_id,
          )
        : null;
      if (finalAsset) {
      ...
      const sourceImages = loadedAssets
        .filter(
          (asset) => asset.category === "source_image" && asset.album_visible,
        )
        .sort(
          (a, b) =>
  ...
  return (
    <main className="private-film-page">
      <header className="private-film-nav">
        ...
        <Link href={`/studio?order=${order.id}`}>制作室へ戻る</Link>
      </header>
      <section className="private-film-hero">
        <div className="private-film-glow" />
        <div>
          <p>PRIVATE MOVING STORYBOOK · {order.order_number}</p>
          <h1>{delivery?.title || `${order.pet_name}の、小さなものがたり`}</h1>
          <span>
            {order.breed} · {order.purpose}
          </span>
        </div>
      </section>

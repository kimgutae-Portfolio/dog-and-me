...
export function StoryWizard() {
  ...
  const missingFields = useMemo<MissingField[]>(() => {
    ...
    if (photoFiles.length === 0) missing.push({ key: "photos", label: "写真（1枚以上）", step: 2 });
    if (!draft.consent) missing.push({ key: "consent", label: "内容と写真の取り扱いへの同意", step: 4 });
    return missing;
    ...
    <main className="wizard-page">
      ...
      <div className="wizard-layout">
        ...
        <section className="wizard-main" aria-labelledby="step-title">
          ...
          {step === 2 && <div className="wizard-panel"><p className="eyebrow">MEMORIES &amp; PHOTOS</p><h1 id="step-title">覚えていることを、少しずつ。</h1><p className="step-lead">{draft.purpose === "虹の橋メモリアル" ? "今は空欄でも次へ進めます。最後の時期や病院のことは任意で、望まない限り映像に入れません。" : "今は空欄でも次へ進めます。最後の確認画面で、必要な項目をまとめてご案内します。"}</p><div className="stacked-fields"><label><span>はじめて会った日のこと <small>任意</small></span><textarea rows={4} value={draft.firstMeeting} onChange={(event) => update("firstMeeting", event.target.value)} placeholder="どこで、どんな表情で出会いましたか？" /></label><label><span>いちばん思い出に残っている時間 <em>必須</em></span><textarea required rows={4} value={draft.favoriteMemory} onChange={(event) => update("favoriteMemory", event.target.value)} placeholder="いつもの散歩道、旅行、家での何気ない時間など" /></label><label><span>その子へ伝えたいこと <em>必須</em></span><textarea required rows={3} value={draft.message} onChange={(event) => update("message", event.target.value)} placeholder="映画の最後に残したい言葉があれば" /></label><label><span>映像に入れたくないこと <small>任意</small></span><textarea rows={2} value={draft.avoid} onChange={(event) => update("avoid", event.target.value)} placeholder="病院の場面、最後の時期、直接的な表現など。遠慮なく書いてください" /></label></div><div className="upload-box"><input id="photos" type="file" required accept="image/jpeg,image/png,image/webp,image/heic,image/heif" multiple onChange={handlePhotos} /><label htmlFor="photos"><span className="upload-mark" aria-hidden="true">＋</span><strong>写真を選ぶ <em>必須・1枚以上</em></strong><small>JPG・PNG・HEIC・WebP / 最大20枚</small></label>{draft.photoNames.length > 0 && <div className="file-list"><strong>{draft.photoNames.length}枚を選択しました</strong>{draft.photoNames.slice(0, 5).map((name) => <span key={name}>{name}</span>)}{draft.photoNames.length > 5 && <span>ほか {draft.photoNames.length - 5}枚</span>}<p>HEICは送信時にJPGへ変換し、写真はお客様専用の非公開領域へ保存します。</p></div>}</div></div>}
          ...
          {step === 4 && <div className="wizard-panel"><p className="eyebrow">REVIEW</p><h1 id="step-title">ありがとうございます。</h1><p className="step-lead">まずは相談受付としてお預かりします。決済は内容と納期をご確認いただいた後です。</p><div className="review-card"><div className="review-title"><span className="brand-mark" aria-hidden="true">WM</span><div><strong>{draft.petName || "愛犬"}ちゃんの映画</strong><small>{draft.purpose}・{draft.style}</small></div></div><dl><div><dt>お名前</dt><dd>{draft.petName || "未入力"}</dd></div><div><dt>犬種・年齢</dt><dd>{[draft.breed, draft.age].filter(Boolean).join(" / ") || "未入力"}</dd></div><div><dt>性格</dt><dd>{draft.personality.join("、") || "未入力"}</dd></div><div><dt>希望</dt><dd>{draft.ratio}・BGM：{draft.bgm}</dd></div><div><dt>写真</dt><dd>{draft.photoNames.length ? `${draft.photoNames.length}枚を選択` : "未入力"}</dd></div><div><dt>プラン</dt><dd>メモリーフィルム</dd></div><div><dt>料金</dt><dd className="review-monitor-price"><strong>先着{MEMORY_FILM_PRICING.launchLimit}組 ¥{formatYen(MEMORY_FILM_PRICING.launchPrice)}（税込）</strong><small>受付時に自動確定・終了後は ¥{formatYen(MEMORY_FILM_PRICING.regularPrice)}</small></dd></div><div><dt>映画の種類</dt><dd>{selectedPurpose.title}</dd></div><div><dt>コンセプト</dt><dd>2案から1案を選択</dd></div><div><dt>共通エンディング</dt><dd>{selectedPurpose.endingTitle}</dd></div><div><dt>専用サイト</dt><dd>プランに含まれます</dd></div></dl><button type="button" className="review-edit" onClick={() => goToStep(0)}>最初から内容を見直す</button></div><label className="consent-box"><input type="checkbox" checked={draft.consent} onChange={(event) => update("consent", event.target.checked)} /><span><strong>内容と写真の取り扱いについて確認しました <em>必須</em></strong><small>使用権限を持つ写真のみを提出し、制作のため非公開領域で取り扱われることに同意します。AI学習には使用しません。</small></span></label>{missingFields.length > 0 ? <aside className="missing-fields-panel" role="status" aria-labelledby="missing-fields-title"><p className="eyebrow">REQUIRED ITEMS</p><h2 id="missing-fields-title">あと{missingFields.length}項目の入力が必要です。</h2><p>項目を選ぶと入力する画面へ戻れます。すべて入力すると、ご相談を送信できます。</p><ul>{missingFields.map((item) => <li key={item.key}><button type="button" onClick={() => goToStep(item.step)}><span>{steps[item.step]}</span><strong>{item.label}</strong><em>入力する →</em></button></li>)}</ul></aside> : <aside className="ready-to-submit" role="status"><span aria-hidden="true">✓</span><div><strong>必要な項目がすべて揃いました。</strong><small>下のボタンからご相談を送信できます。</small></div></aside>}</div>}

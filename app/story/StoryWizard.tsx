...
export function StoryWizard() {
  ...
  useEffect(() => {
    ...
    const plan = searchParams.get("plan") ?? "film";
    if (stored) {
  ...
  return (
    <main className="wizard-page">
      <header className="wizard-header">
        <Link className="brand" href="/" aria-label="トップへ"><span className="brand-mark" aria-hidden="true">き</span><span className="brand-type">きみとの映画<small>OUR PAW FILM</small></span></Link>
      ...
      <div className="wizard-layout">
        ...
        <section className="wizard-main" aria-labelledby="step-title">
          ...
          {step === 4 && <div className="wizard-panel"><p className="eyebrow">REVIEW</p><h1 id="step-title">ありがとうございます。</h1><p className="step-lead">まずは相談受付としてお預かりします。決済は内容と納期をご確認いただいた後です。</p><div className="review-card"><div className="review-title"><span className="brand-mark" aria-hidden="true">き</span><div><strong>{draft.petName || "愛犬"}ちゃんの映画</strong><small>{draft.purpose}・{draft.style}</small></div></div><dl><div><dt>お名前</dt><dd>{draft.petName || "未入力"}</dd></div><div><dt>犬種・年齢</dt><dd>{[draft.breed, draft.age].filter(Boolean).join(" / ") || "未入力"}</dd></div><div><dt>性格</dt><dd>{draft.personality.join("、") || "未入力"}</dd></div><div><dt>希望</dt><dd>{draft.ratio}・{draft.narration}</dd></div><div><dt>写真</dt><dd>{draft.photoNames.length ? `${draft.photoNames.length}枚を選択` : "後から追加"}</dd></div><div><dt>プラン</dt><dd>{draft.plan === "short" ? "メモリーショート" : draft.plan === "memorial" ? "メモリアル" : "メモリーフィルム"}</dd></div></dl><button type="button" className="review-edit" onClick={() => setStep(0)}>最初から内容を見直す</button></div><label className="consent-box"><input type="checkbox" checked={draft.consent} onChange={(e) => update("consent", e.target.checked)} /><span><strong>内容と写真の取り扱いについて確認しました</strong><small>自分が使用権限を持つ写真のみを提出し、制作目的で取り扱われることに同意します。1次版では外部送信されません。</small></span></label></div>}

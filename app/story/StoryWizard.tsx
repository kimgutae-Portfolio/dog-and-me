...
// The output format is fixed: a near-live-action dog with softly painterly
// surroundings, 16:9, no narration, and BGM chosen by the director.
const FIXED_FILM_STYLE = "実写に近い愛犬と、やわらかな絵画表現";
...
export function StoryWizard() {
  ...
  const missingFields = useMemo<MissingField[]>(() => {
    ...
    if (!draft.aiReconstructionAcknowledged) missing.push({ key: "aiReconstructionAcknowledged", label: "仕上がりの表現についての確認", step: 3 });
    return missing;
    ...
    <main className="wizard-page">
      ...
      <div className="wizard-layout">
        ...
        <section className="wizard-main" aria-labelledby="step-title">
          ...
          {step === 0 && <div className="wizard-panel"><p className="eyebrow">ABOUT YOUR DOG</p><h1 id="step-title">その子のことを教えてください。</h1><p className="step-lead">「必須」と表示された項目をすべて入力すると、次のステップへ進めます。</p><div className="form-grid"><label><span>お名前 <em>必須</em></span><input required value={draft.petName} onChange={(event) => update("petName", event.target.value)} placeholder="例：ひなた" /></label><label><span>お名前の読み方 <small>任意</small></span><input value={draft.nameKana} onChange={(event) => update("nameKana", event.target.value)} placeholder="例：ひなた" /></label><label><span>犬種 <em>必須</em></span><input required value={draft.breed} onChange={(event) => update("breed", event.target.value)} placeholder="例：柴犬" /></label><label><span>年齢 <em>必須</em></span><input required value={draft.age} onChange={(event) => update("age", event.target.value)} placeholder="例：4歳 / 推定3歳" /></label></div><fieldset className="chip-field"><legend>どんな性格ですか？ <small>1つ以上・必須</small></legend><div>{personalities.map((personality) => <button type="button" className={draft.personality.includes(personality) ? "chip selected" : "chip"} onClick={() => togglePersonality(personality)} key={personality}>{personality}<span aria-hidden="true">＋</span></button>)}</div></fieldset></div>}
          ...
          {step === 2 && <div className="wizard-panel"><p className="eyebrow">YOUR MEMORIES</p><h1 id="step-title">覚えていることを、少しずつ。</h1><p className="step-lead">3つの思い出をお聞かせください。担当ディレクターがそれぞれの内容から複数の場面を組み立て、2つの映像構成案としてご提案します。制作前には場面イメージもご確認いただけます。</p><section className="memory-writing-guide" aria-labelledby="memory-writing-guide-title"><div><p className="eyebrow">WRITING GUIDE</p><h2 id="memory-writing-guide-title">映像にしやすい伝え方</h2></div><ol><li><span>01</span><div><strong>ひとつの出来事に絞る</strong><p>「旅行」だけではなく「海辺で初めて波を見た日」のように、ひとつの出来事として教えてください。そこから複数の場面をご提案します。</p></div></li><li><span>02</span><div><strong>その子の動きや表情も一緒に書く</strong><p>走った、振り返った、首をかしげたなど、実際に見た様子を文章の中に含めてください。</p></div></li><li><span>03</span><div><strong>写真があれば参考として添える</strong><p>同じ出来事の写真があれば任意で選べます。なくても、文章と基準写真から場面をおつくりします。</p></div></li></ol><p>例：「去年の春、いつもの公園で桜を見ました。ひなたは花びらを追いかけたあと、こちらを見て首をかしげました。」</p></section>

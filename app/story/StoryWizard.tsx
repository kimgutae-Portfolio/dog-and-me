  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        setDraft({ ...emptyDraft, ...parsed, purpose });
    ...
    <main className="wizard-page">
      ...
      <div className="wizard-layout">
        ...
        <section className="wizard-main" aria-labelledby="step-title">
          ...
          {step === 2 && <div className="wizard-panel"><p className="eyebrow">MEMORIES &amp; PHOTOS</p><h1 id="step-title">覚えていることを、少しずつ。</h1><p className="step-lead">{draft.purpose === "虹の橋メモリアル" ? "答えにくいことは飛ばして大丈夫です。最後の時期や病院のことを、望まない限り映像に入れません。" : "全部埋めなくても大丈夫です。言葉にならないところは空欄のままで。"}</p><div className="stacked-fields"><label><span>はじめて会った日のこと</span><textarea rows={4} value={draft.firstMeeting} onChange={(e) => update("firstMeeting", e.target.value)} placeholder="どこで、どんな表情で出会いましたか？" /></label><label><span>いちばん思い出に残っている時間</span><textarea rows={4} value={draft.favoriteMemory} onChange={(e) => update("favoriteMemory", e.target.value)} placeholder="いつもの散歩道、旅行、家での何気ない時間など" /></label><label><span>その子へ伝えたいこと</span><textarea rows={3} value={draft.message} onChange={(e) => update("message", e.target.value)} placeholder="映画の最後に残したい言葉があれば" /></label><label><span>映像に入れたくないこと</span><textarea rows={2} value={draft.avoid} onChange={(e) => update("avoid", e.target.value)} placeholder="病院の場面、最後の時期、直接的な表現など。遠慮なく書いてください" /></label></div><div className="upload-box"><input id="photos" type="file" accept="image/jpeg,image/png,image/webp,image/heic" multiple onChange={handlePhotos} /><label htmlFor="photos"><span className="upload-mark" aria-hidden="true">＋</span><strong>写真を選ぶ</strong><small>JPG・PNG・HEIC・WebP / 1次版は最大20枚</small></label>{draft.photoNames.length > 0 && <div className="file-list"><strong>{draft.photoNames.length}枚を選択しました</strong>{draft.photoNames.slice(0, 5).map((name) => <span key={name}>{name}</span>)}{draft.photoNames.length > 5 && <span>ほか {draft.photoNames.length - 5}枚</span>}<p>※ 1次版ではファイル名のみ一時保存されます。正式送信はStorage接続後に有効化します。</p></div>}</div></div>}

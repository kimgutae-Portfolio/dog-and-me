...
export async function POST(request: NextRequest) {
  ...
  if (
    !Number.isSafeInteger(order.quoted_price) ||
    order.quoted_price < 100 ||
    order.quoted_price > 1_000_000
  ) {
  ...
  if (activeSession?.stripe_session_id) {
    const existing = await stripe.checkout.sessions.retrieve(
      activeSession.stripe_session_id,
    );
    ...
    if (Date.now() - createdAt < 5 * 60 * 1000) {
      return NextResponse.json(
        { error: "checkout_is_being_prepared" },
        { status: 409 },
      );
    }
    await admin
  ...
  if (reservationError || !reservation) {
    const { data: concurrentSession, error: concurrentSessionError } =
      await admin
        .from("stripe_checkout_sessions")
    ...
    if (concurrentSession?.stripe_session_id) {
      const existing = await stripe.checkout.sessions.retrieve(
        concurrentSession.stripe_session_id,
      );
      if (existing.status === "open" && existing.url)
        return NextResponse.json({ url: existing.url });
    }
        ...
        line_items: [
          {
            ...
            price_data: {
              ...
              product_data: {
                name: "WAN MEMORY うちの子の動く絵本",
                description: `${order.pet_name}ちゃんの動く絵本制作（${order.order_number}・税込）`,
              },
            },
          },
        ],

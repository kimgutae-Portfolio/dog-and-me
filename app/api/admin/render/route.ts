...
import type { RenderClipRole, RenderProgressEvent } from "../../../lib/supabase/types";
...
async function downloadTo(
  ...
  if (error || !data) throw new Error(`ダウンロードに失敗しました: ${storagePath}`);
 ...
 */
function runAssembler(args: string[], onProgress: (percent: number, label: string) => void) {
  return new Promise<void>((resolve, reject) => {
    const child = spawn("python3", [ASSEMBLE_SCRIPT, ...args], { cwd: process.cwd() });
    let stderrTail = "";
    ...
    child.on("error", (cause) => reject(new Error(`python3 を起動できませんでした: ${cause.message}`)));
    child.on("close", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`編集処理が失敗しました (exit ${code})\n${stderrTail.trim().slice(-1200)}`));
    });
  });
}
...
export async function POST(request: NextRequest) {
  if (process.env.WM_LOCAL_RENDER !== "1") {
    return Response.json(
      {
        error: "local_only",
        message: "映像の編集はローカルの制作環境でのみ実行できます。`npm run dev:operator` で起動してください。",
      },
      { status: 501 },
    );
  }
  ...
  if (!isUuid(orderId)) return Response.json({ error: "invalid_order" }, { status: 400 });

  const rawItems = Array.isArray(payload.items) ? payload.items : [];
  const items: RequestItem[] = [];
  for (const raw of rawItems) {
    const item = raw as Partial<RequestItem>;
    if (!isUuid(item.clipAssetId)) return Response.json({ error: "invalid_clip" }, { status: 400 });
    if (item.role !== "intro" && item.role !== "memory" && item.role !== "ending") {
  ...
  if (items[0].role !== "intro" || items.filter((i) => i.role === "intro").length !== 1) {
    return Response.json({ error: "invalid_intro" }, { status: 400 });
  }
  if (items[items.length - 1].role !== "ending" || items.filter((i) => i.role === "ending").length !== 1) {
  ...
  const kicker = typeof payload.kicker === "string" ? payload.kicker.trim() : "A MEMORY FILM";
  const endingText = typeof payload.endingText === "string" ? payload.endingText.trim() : "";
  const endingMark = typeof payload.endingMark === "string" ? payload.endingMark.trim() : "WAN MEMORY";
  if (!title || title.length > 80) return Response.json({ error: "invalid_title" }, { status: 400 });
  if (!endingText || endingText.length > 600) return Response.json({ error: "invalid_ending_text" }, { status: 400 });

  const letterboxPct = typeof payload.letterboxPct === "number" ? payload.letterboxPct : 0;
  if (letterboxPct < 0 || letterboxPct > 15) return Response.json({ error: "invalid_letterbox" }, { status: 400 });
  ...
  const { data: clipRows, error: clipError } = await supabase
    .from("assets")
    .select("id, order_id, category, storage_path, scene_sort_order, source_still_asset_id")
    .eq("order_id", orderId)
    .eq("category", "render_clip");
  if (clipError) return Response.json({ error: "clip_lookup_failed" }, { status: 400 });

  const clipById = new Map((clipRows as ClipRow[] | null ?? []).map((row) => [row.id, row]));
  const ordered: ClipRow[] = [];
  for (const item of items) {
    const clip = clipById.get(item.clipAssetId);
    if (!clip) return Response.json({ error: "clip_not_in_order" }, { status: 400 });
    if (!clip.source_still_asset_id) return Response.json({ error: "clip_missing_still" }, { status: 400 });
    ordered.push(clip);
  }
  ...
  const { data: stillRows, error: stillError } = await supabase
    .from("assets")
    .select("id, storage_path, category, order_id")
    .in("id", stillIds);
  if (stillError) return Response.json({ error: "still_lookup_failed" }, { status: 400 });
  const stillById = new Map(
    (stillRows as { id: string; storage_path: string; category: string; order_id: string }[] | null ?? [])
      .map((row) => [row.id, row]),
  );
  for (const id of stillIds) {
    const still = stillById.get(id);
    if (!still || still.category !== "scene_still" || still.order_id !== orderId) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      const send = (event: RenderProgressEvent) => controller.enqueue(ndjson(event));
      ...
      try {
        ...
        send({ type: "progress", step: "download", message: "素材を読み込んでいます" });
        for (let index = 0; index < ordered.length; index += 1) {
          const clip = ordered[index];
          const still = stillById.get(clip.source_still_asset_id as string)!;
          const n = index + 1;
          await downloadTo(supabase, still.storage_path, path.join(workDir, `${n}.png`));
          await downloadTo(supabase, clip.storage_path, path.join(workDir, `${n}-video.mp4`));
          send({
            type: "progress",
            step: "download",
        ...
        const memoryClips = ordered.slice(1, -1).map((_, index) => String(index + 2));
        const args = [
          "--order-dir", workDir,
          "--intro-clip", "1",
          "--memory-clips", ...(memoryClips.length ? memoryClips : ["1"]),
          "--ending-clip", String(ordered.length),
          "--kicker", kicker,
          "--title", title,
          "--ending-text", endingText.replace(/\n/g, "\\n"),
          "--ending-mark", endingMark,
          "--out", outPath,
        ];
        if (letterboxPct > 0) args.push("--letterbox", "--letterbox-pct", String(letterboxPct));
        if (filmLook) args.push("--film-look");
        if (bgmFile) args.push("--bgm", path.join(BGM_DIR, bgmFile));

        await runAssembler(args, (percent, label) => {
          send({ type: "progress", step: "assemble", message: `${label} (${percent}%)` });
        });

        send({ type: "progress", step: "upload", message: "完成した映像を保存しています" });
        ...
        if (uploadError) throw new Error(`保存に失敗しました: ${uploadError.message}`);
        uploadedPath = storagePath;

        const durationSeconds = 3.0 + ordered.length * 6.5 + 7.0 - 0.5 * (2 * ordered.length + 1);
        const { data: assetId, error: registerError } = await supabase.rpc("admin_register_assembled_film", {
          p_order_id: orderId,
        ...
        send({ type: "done", assetId: assetId as string, durationSeconds, fileSize: size });
      } catch (cause) {
        if (uploadedPath) await supabase.storage.from(BUCKET).remove([uploadedPath]).catch(() => {});
        send({ type: "error", message: cause instanceof Error ? cause.message : "編集に失敗しました" });
      } finally {

-- Story motion now uses one continuous Runway clip per story:
-- three selected important stories are 10 seconds and the other two are 5 seconds.
-- Keep the render_take column for compatibility with existing rows, but stop
-- creating or requiring a second take in the production workflow.
...
begin
  ...
  if p_render_take <> 1 then raise exception 'only one continuous render clip is supported'; end if;
  ...
  if exists (
    ...
    raise exception 'this story already has a render clip';
  end if;
  ...
  values (
    ...
    v_still.scene_sort_order, p_still_asset_id, 1
  )
